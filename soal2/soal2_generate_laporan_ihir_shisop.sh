...
(echo "Daftar nama customer di Alburquerque pada tahun 2017 antara lain:"; awk -F "\t" '/Albuquerque/&&$3 ~ /.....-17/ NR > 1  { print $7}' Laporan-TokoShiSop.tsv | uniq) >> hasil.txt
...
BEGIN {FS = "\t";} 
{
    if(NR != 1){
		...
		if($8 == kat[1]){tr[1] += 1;}
		if($8 == kat[2]){tr[2] += 1;}
		for (t in kat){if(tr[t] <= trmin){ke = t;}}
		ar[0]="Central";
		ar[1]="East";
		ar[2]="South";
		ar[3]="West";
		...
		if($13 == ar[2]){par[2] += $21;}
		if($13 == ar[3]){par[3] += $21;}
		for(r in par){if(par[r] <= pmin){kep = r;}}
	}
}
END {
